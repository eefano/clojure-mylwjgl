...
(defn loadcl [env]
  (let [
        ...
        ret (BufferUtils/createByteBuffer 4)
        program (CL10/clCreateProgramWithSource (:context env) src (.asIntBuffer ret))
        rt1 (.get (.asIntBuffer ret) 0)
        rt2 (CL10/clBuildProgram ^Long program ^Long (:device env) "" nil 0)
        kernel (CL10/clCreateKernel program "sum" (.asIntBuffer ret))
        rt3 (.get (.asIntBuffer ret) 0)
        ]
